...
5. **作業**: BGMを流しながら使えるポモドーロタイマー(25分/50分 × 1〜4セット。休憩明けは次のセットが自動で始まります)と、やることリスト(期限・繰り返し=毎日/毎週/毎月を設定可。繰り返しは完了すると次の期限のタスクが自動で作られます。並びは登録順/期限順、絞り込みは すべて/今日まで/今週まで/期限あり)。日ごとの作業時間・完了数は「作業のあゆみ」で振り返れます(‹ ›で過去の週へ、日をタップでその日に完了したやることの内容も見られます)。タイマー進行中は「今日の曲」画面の先頭にも残り時間が出ます。設定で「開いたらタイマーを自動で開始」(時間・セット数も事前設定)にもできます
...
| `bgm_todos` | やることリスト `{id: {text, done, createdAt, doneAt, updatedAt, deleted, due, repeat, nextId}}`(期限・繰り返し対応。削除はトンボストーン=30日で掃除。完了は内容の振り返り用に90日保持) |
| `bgm_todoview` | やることリストの表示設定 `{sort, filter}`(登録順/期限順・すべて/今日まで/今週まで/期限あり。端末ローカル) |
